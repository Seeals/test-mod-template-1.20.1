...
public class GemPolishingScreen extends HandledScreen<GemPolishingScreenHandler> {
    ...
    private void renderProgressArrow(DrawContext context, int x, int y) {
       if (handler.isCrafting()) {
           context.drawTexture(TEXTURE, x + 85, y + 30, 176, 0, 8, handler.getScaledProgress());          //If the scaled progress is horizontal the handler.getScaledProgress wouldn't be at the height, but at the width.
       }
    }
